// Netlify Function — mirrors backend/server.js for production.
// Local dev still uses the Express server via the CRA proxy.
// Env vars (GITHUB_PAT, PUBLISH_API_SECRET) are set in Netlify dashboard.
...
exports.handler = async (event) => {
  ...
  if (!publishSecret) {
    return json(500, { error: 'Server publish secret missing. Set PUBLISH_API_SECRET in Netlify environment variables.' });
  }

  // Netlify lowercases all incoming headers.
  const headerSecret = event.headers['x-publish-secret'] || '';
  if (headerSecret !== publishSecret) {
    return json(401, { error: 'Unauthorized publish request.' });
  }

  if (!token) {
    return json(500, { error: 'Server token missing. Set GITHUB_PAT in Netlify environment variables.' });
  }
  ...
  try {
    ...
    await githubCommitFile(
      token, owner, repo, branch, galleryDataPath,
